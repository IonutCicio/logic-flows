...
<div>
    <div class="relative">
        ...
        {#if selectedComponent}
            <div class="absolute top-5 left-5 bg-white border w-50 h-4/5">
                <PropertyInspector component={selectedComponent.model} graph={graph} />
            </div>
        {/if}
    ...
    {/if}
    {#if editMode === EditMode.AddGeneralization}
        <div class="absolute right-0 top-0 w-72 bg-white h-full">
            <GeneralizationInstantiator classes={graph?.getCells().filter(c => c instanceof JointJSClass) ?? []} onCreate={addGeneralizationToGraph}/>
        </div>
    {/if}
</div>
