...
<div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white rounded-xl p-6 w-full max-w-md mx-4 shadow-2xl">
        <h3 class="text-2xl font-semibold text-center mb-6">
            {property
                ? `Edit ${kind === "attribute" ? "attribute" : "operation"}`
                : `New ${kind === "attribute" ? "attribute" : "operation"}`}
        </h3>
        ...
        <div class="flex gap-3 mt-8">
            <button
                onclick={onClose}
                class="flex-1 py-2.5 border rounded-lg hover:bg-gray-50"
                >Cancel</button
            >
            <button
                onclick={handleSave}
                class="flex-1 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
                >Save</button
            >
        </div>
    </div>
