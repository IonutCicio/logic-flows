<script lang="ts">
    ...
    let attributes: ManuallyOrderedMap<string, UMLAttributeData> = $state(
        new ManuallyOrderedMap(),
    );
    let attributes_entries = $derived(attributes.entries());
    ...
    <div>
        <div class="flex justify-between items-center mb-2">
            ...
            <button
                ...
                onclick={() => {
                    const attributes_with_default_name_count =
                        attributes_entries.filter(([name, _]) =>
                            name.startsWith("attribute"),
                        ).length;

                    console.log("here", attributes_with_default_name_count);

                    attributes.set(
                        `attribute${attributes_with_default_name_count > 0 ? attributes_with_default_name_count : ""}`,
        ...
        <div class="space-y-1">
            {#each attributes_entries as name, index}
                <div
